...
def gerar_cards_os(df_mes, conc, caminho_arquivo):
    # Cálculos
    total_os = len(df_mes)
    no_tempo = len(df_mes[df_mes['StatusTempo'] == 'No Tempo'])
    fora_tempo = len(df_mes[df_mes['StatusTempo'] == 'Fora do Tempo'])

    pct_no = round((no_tempo / total_os) * 100, 2) if total_os > 0 else 0
    pct_fora = round((fora_tempo / total_os) * 100, 2) if total_os > 0 else 0
    total_os_fmt = f"{total_os:,}".replace(",", ".")

    # Cores e textos
    cores = ["#001f4d", "#2e7d32", "#f57c00"]
    textos = [("Qtde Serviços", total_os_fmt),
              ("Serviços No Tempo", f"{pct_no:.2f}%"),
              ("Serviços Fora do Tempo", f"{pct_fora:.2f}%")]
