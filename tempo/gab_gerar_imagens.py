...
def main():
    ...
    # 🔹 FILTRO GLOBAL DAS CONCESSIONÁRIAS + COPY (IGUAL AO GAB PRAZO)
    df = df[df['EMPRESA'].isin(CONCESSIONARIAS)].copy()

    df['TempoPadrao'] = df.apply(
        lambda x: gab_calcular_tempo_dax(x, df, df_servicos),
        axis=1
    )
